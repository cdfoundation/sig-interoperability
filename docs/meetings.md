...
* Open Disscussion
    ...
    * Interoperability discussions on CDEvents
        * Melissa: Ensure we are not overlapping too much with CDEvents SIG; supporting but not closing door to additional ideas that might come up
        * Andrea: CDEvents SIG is focused more on technical aspects, support with whitepaper and example ref arch very much needed/appreciated
        * Dadisi: CDEvents is "the thing" we currently have to show Interoperability in action, so keeping Interoperability SIG updated and involved is very much relevant and we should continue
