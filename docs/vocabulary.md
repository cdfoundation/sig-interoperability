...
|-----------|---------------|---------------|----------|----------------------|
| **Setup** | Provision build resources, set up the build workspace. | Initialize, Start, Prepare, Workspace | Build Request Parameters, Build Container Image Name and Version | Secrets, Build Environment, Build Workspace |
| **Source** | Download, retrieve or copy software, images, and documentation into the build workspace.  | N/A | Source Code Repository, Source Code Branch or Commit | Software Artifacts, Container Images |
| **Detect Secrets** | Detect secrets in the source code, other software, or documentation.  | N/A | Source Code Repository, Source Code Branch or Commit, Software Artifacts, Container Images | Secret Detection Report, Revoked Secrets |
| **Policy Check** | Verifies that policies are followed, for example:  software is from a trusted source, source repositories are configured correctly, dependencies are signed, code standards are followed, code reviews are completed, there is a secure chain of custody, or appropriate work items or change requests are associated with the change. | Provenance Check | Source Code Repository, Source Code Branch or Commit, Software Artifacts, Container Images | Policy and/or Provenance Reports |
| **Build** | Assemble and/or compile software and documentation into an executable and usable format. | Compile, Install, Assemble | Software Artifacts, Container Images | Container Images, Compiled Documentation, Executable Software |
| **Test** | Run the default test suite for the build. Typically includes unit tests, linting, and low-level integration tests that don't require connecting to a deployed server. | Unit Test | Container Images, Compiled Documentation, Executable Software | Test/Lint Results, Test/Lint Reports, Test/Lint Coverage Reports |
...
| **Scan** | Use a tool to do verification of software and documenation other than testing. Examples include static code analysis, checking for known vulnerabilities in code or binaries, dynamic security scans, license checks, and code smells. | N/A | Software Artifacts, Container Images, Compiled Documentation, Executable Software | Scan Results, Scan Reports, Scan Coverage Reports |
| **Package** | Create the software artifact(s) that will be published. Typically updates the software version. | N/A | Software Artifacts, Container Images, Compiled Documentation, Executable Software | Packaged Software or Documentation, Images, Binaries, Archives |
| **Bill of Materials** | Construct a Software Bill of Materials (SBOM) that includes both direct and indirect dependencies that are included in the software and documentation as packaged. | SBOM | Software Artifacts, Container Images, Compiled Documentation, Executable Software | SBOM |
...
| **Provision** | Request that a new environment, infrastructure, or other resources be provisioned, for example, a test cluster or object storage. | Obtain, Request | Resource Request Parameters | Provisioned Environment(s)/Resource(s)  |
| **Deploy** | Make changes to any environment other than the build environment. Configure the environment. Deploy dependencies, software artificts and/or documentation. | N/A   | Software Artifacts, Container Images, Compiled Documentation, Executable Software | Software running in another environment. Documentation hosted in another environment. Routes to Deployments. Deployment Records. Secrets to access deployed resources. |
| **Verify Deployment** | Verify successful deployment of software or documentation. | Smoke Test | Routes to Deployments. Secrets to access deployed resources. | Deployment Verification Results, Deployment Verification Records |
...
| **Create Request** | Create a request in another system; for example, a Change Request that must be approved for a deployment to production. | Approval, Ticket | Output of previous steps. | Request Created, Link to Request |
| **Update Request** | Update a request in another system; for example, a Work Item or Change Request that must be updated and closed after a deployment to production. | N/A | Output of previous steps. Link to Request | Request Updated |
| **Record and Report Results** | Final recording and reporting of build results. Upload build logs and other artifacts for long-term archival. | Audit, Evidence | Output of previous steps. | Compliance Evidence, Build Results, Build Reports, Build Archives. |
| **Cleanup** | Release build resources, de-provision environments, delete build workspace and build container(s). | Finalize   | Output of previous steps. | Deleted Build Environment and Build Workspace  |
